...
interface HistoryDao {
    @Query("SELECT * FROM maintenance_history ORDER BY completedAt DESC")
    fun observeAll(): Flow<List<MaintenanceHistoryEntity>>

    @Query("SELECT * FROM maintenance_history WHERE assetId = :assetId ORDER BY completedAt DESC")
    fun observeForAsset(assetId: String): Flow<List<MaintenanceHistoryEntity>>

    @Query("SELECT * FROM maintenance_history")
    suspend fun getAll(): List<MaintenanceHistoryEntity>
