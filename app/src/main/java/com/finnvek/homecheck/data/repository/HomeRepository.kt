...
class HomeRepository
    ...
    constructor(
        ...
        suspend fun upsertAsset(asset: AssetEntity) = mutationGate.withLock { assetsDao.upsert(asset) }

        suspend fun upsertTask(task: MaintenanceTaskEntity) = mutationGate.withLock { tasksDao.upsert(task) }

        suspend fun upsertAttachment(attachment: AttachmentEntity) = mutationGate.withLock { attachmentsDao.upsert(attachment) }

        suspend fun deleteAsset(id: String) = mutationGate.withLock { assetsDao.delete(id) }

        suspend fun deleteTask(id: String) = mutationGate.withLock { tasksDao.delete(id) }

        suspend fun deleteAttachment(id: String) = mutationGate.withLock { attachmentsDao.delete(id) }

        suspend fun renameAttachment(
            id: String,
            name: String,
        ) = mutationGate.withLock { attachmentsDao.rename(id, name) }

        suspend fun changeAttachmentType(
            id: String,
            type: AttachmentType,
        ) = mutationGate.withLock { attachmentsDao.changeType(id, type) }
        ...
        suspend fun undoCompletion(result: CompletionResult) =
            mutationGate.withLock {
                database.withTransaction {
        ...
        suspend fun snapshot(): DatabaseSnapshot =
            mutationGate.withLock {
                database.withTransaction {
                    DatabaseSnapshot(
