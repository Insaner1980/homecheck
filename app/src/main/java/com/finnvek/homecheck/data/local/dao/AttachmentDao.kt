...
interface AttachmentDao {
    ...
    @Query("SELECT * FROM attachments WHERE assetId = :assetId")
    suspend fun getForAsset(assetId: String): List<AttachmentEntity>
    ...
    @Query("UPDATE attachments SET displayName = :name WHERE id = :id")
    suspend fun rename(
        id: String,
        name: String,
    )

    @Query("UPDATE attachments SET type = :type WHERE id = :id")
