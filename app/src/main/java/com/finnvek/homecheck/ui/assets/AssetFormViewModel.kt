...
data class AssetSavedEvent(
    val assetId: String,
    val photoImportFailed: Boolean,
)
...
class AssetFormViewModel
    ...
    constructor(
            ...
            viewModelScope.launch {
                val event =
                    mutationGate.withLock {
                        ...
                        if (runCatching { repository.upsertAsset(asset) }.isFailure) {
                            mutableState.value = form.copy(isSaving = false, saveError = true)
                        ...
                        form.pendingPhotoUri?.let { uriValue ->
                            runCatching {
                                repository
                                    .attachmentsNow(assetId)
                                    .filter { it.type == AttachmentType.ASSET_PHOTO }
                                    .forEach { old ->
                                        repository.deleteAttachment(old.id)
                                        attachmentStore.delete(old.localPath)
                                    }
                        ...
                        AssetSavedEvent(assetId, photoFailed)
                    }
                if (event == null) {
                    return@launch
                }
        ...
        private fun load(asset: AssetEntity) =
            update(
                AssetFormState(
                    assetId = asset.id,
                    name = asset.name,
