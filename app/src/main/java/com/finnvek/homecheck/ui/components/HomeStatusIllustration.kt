...
private const val PATH_MIDPOINT = 0.5f
private const val PATH_PHASE_SCALE = 2f

@Composable
fun HomeStatusIllustration(
    allClear: Boolean,
    ...
    Canvas(modifier.clearAndSetSemantics { }.size(112.dp)) {
        ...
        if (progress <= PATH_MIDPOINT) {
            drawLine(color, a, interpolate(a, b, progress * PATH_PHASE_SCALE), stroke, StrokeCap.Round)
        } else {
            drawLine(color, a, b, stroke, StrokeCap.Round)
            drawLine(color, b, interpolate(b, c, (progress - PATH_MIDPOINT) * PATH_PHASE_SCALE), stroke, StrokeCap.Round)
        }
    }
}
