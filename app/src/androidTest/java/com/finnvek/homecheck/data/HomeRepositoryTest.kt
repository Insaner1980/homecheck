...
class HomeRepositoryTest {
    ...
    @Test fun recurringCompletionCreatesHistoryAndAdvancesFromActualCompletion() = runTest {
    ...
    @Test fun oneTimeCompletionCreatesHistoryAndRemovesActiveTask() = runTest {
        repository.upsertAsset(asset())
        ...
        assertEquals("Clean filter", repository.historyForAsset("asset-1").first().single().titleSnapshot)
    }

    @Test fun deletingAssetCascadesItsDatabaseRecords() = runTest {
        repository.upsertAsset(asset())
        repository.upsertTask(task())
        ...
        assertTrue(database.attachmentDao().observeForAsset("asset-1").first().isEmpty())
